# LANDING-GENERATION.md — Next.js Waitlist Page Templates

This file is the sidecar generation guide for the Landing capability (`references/landing.md`). It contains:

1. Copy generation rules (eyebrow, H1, description, bullets, proof line)
2. The Next.js scaffold templates — every file the capability writes into `landing/`
...
The Landing capability reads this guide before drafting copy or scaffolding files. Treat the templates here as canonical — emit them verbatim with placeholders substituted, do not improvise file contents.
...
- **Empathy-led** — leans on the pain ("Because spending three weekends on a landing page is how good ideas die.")
...
### 2.4 `landing/postcss.config.mjs`
...
### 2.8 `landing/components/FormEmbed.tsx`

See §3 for the full file content.

### 2.9 `landing/app/page.tsx`
...
- **Local image** (founder pasted a path; copied into `landing/public/hero.<ext>`):
  ```tsx
  <Image
    src="/hero.{{ext}}"
    alt=""
...
### 2.11 `landing/README.md`

```markdown
# {{product_name}} — Landing Page

Generated by `/plaid landing` from `docs/product-idea.md` and `docs/design.md`.
...
Re-run `/plaid landing` (regenerate mode) to refresh after editing `design.md`.
...
### `{{provider_id_instructions}}` for `landing/README.md`

| Provider     | Instructions to print in `landing/README.md`                                                                  |
|--------------|----------------------------------------------------------------------------------------------------------------|
...
Read the YAML front matter of `docs/design.md`. For each token present, emit one line into the `@theme` block of `landing/app/globals.css`. **Only emit tokens that exist in the YAML — do not invent.**
