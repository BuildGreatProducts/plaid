...
description: |
  ...
  into a design.md spec), Landing (scaffold a Next.js waitlist page from
  ...
  "create design.md", "plaid landing", "scaffold a landing page",
  "waitlist page", "plaid launch", "go-to-market", "GTM strategy",
...
| "plaid landing", "scaffold a landing page", "waitlist page", "build a waitlist", "make a landing page", "convert design to landing" | `references/landing.md` |
...
- `docs/product-idea.md` and `docs/design.md` both exist, AND `landing/` does not exist, AND the user mentions "landing", "waitlist", "preview page", "marketing page", "go live with a page", or "scaffold a page" → route to Landing
...
Landing is also orthogonal — it pairs with Launch but runs any time `docs/product-idea.md` and `docs/design.md` both exist. It scaffolds a Next.js waitlist page in `landing/`. Route to it whenever the founder's intent centers on capturing waitlist signups or publishing a marketing page.
...
- After Plan completes → suggest Design (`/plaid design`) if the founder has imagery to anchor on; if `docs/product-idea.md` and `docs/design.md` are both already present, suggest Landing (`/plaid landing`) as a fast next step before launching (`/plaid launch`) or building (`/plaid build`)
- After Design completes → if `docs/product-idea.md` exists, suggest Landing (`/plaid landing`) as the next concrete asset; otherwise, if `docs/prd.md` does not exist, suggest Plan (`/plaid plan`); if `docs/prd.md` exists, suggest Build (`/plaid build`)
- After Landing completes → suggest Launch (`/plaid launch`) — the gtm.md plan references the landing URL, and the landing page is the first GTM channel
- After Launch completes → if `landing/` does not exist but `docs/product-idea.md` and `docs/design.md` both do, suggest Landing (`/plaid landing`) as a quick win before publishing the GTM plan; otherwise suggest building (`/plaid build`)
